...
function loadQuestionBank() {
  ...
  return {
    topics,
    debugged: parseFraudFile(path.join(DATA_DIR, "FRAUD.txt")),
...
function parseQuestionFile(filePath, topicName) {
  ...
  return blocks
    .map((block, index) => parseQuestionLines(block.split("\n"), topicName, index + 1, true))
    .filter(Boolean);
}

function parseFraudFile(filePath) {
  const raw = fs.readFileSync(filePath, "utf8").replace(/\r/g, "");
  const lines = raw
    .split("\n")
    .map((line) => line.trim())
    .filter(Boolean);
  const questions = [];
  let topic = "FEU Alabang";
  let block = [];

  const flush = () => {
    const parsed = parseQuestionLines(block, topic, questions.length + 1, false);
    if (parsed) questions.push(parsed);
    block = [];
  };

  for (const line of lines) {
    const heading = fraudTopicName(line);
    if (heading) {
      flush();
      topic = heading;
      continue;
    }
    if (block.some((item) => /^correct answer\s*:/i.test(item)) && !isChoiceLine(line) && !/^correct answer\s*:/i.test(line)) {
      flush();
    }
    block.push(line);
  }
...
function fraudTopicName(line) {
  const clean = String(line || "").trim().replace(/\s*\(.*\)\s*$/, "");
  if (/^java$/i.test(clean)) return "Java";
  if (/^gaming$/i.test(clean)) return "Gaming";
  if (/^feu alabang$/i.test(clean)) return "FEU Alabang";
  return "";
}
...
function parseQuestionLines(rawLines, topicName, index, allowFallback) {
  ...
  if (!choiceLines.length && !allowFallback) return null;
  const questionLine = lines.find((line) => !isChoiceLine(line) && !/^correct answer/i.test(line));
  if (!questionLine && !allowFallback) return null;
  const question = cleanQuestion(questionLine || `Question ${index}`);
...
function sanitizeGame(room, self, isHost) {
  ...
  if (room.mode === "fraud") {
    ...
    return {
      ...
      transitionEnd: game.transitionEnd,
      revealEnd: game.revealEnd,
      scoreboardEnd: game.scoreboardEnd,
      ejectionEnd: game.ejectionEnd,
      tieBreakEnd: game.tieBreakEnd,
      nextTopic: game.nextTopic || "",
      ejectedId: game.ejectedId || null,
      ejectedRole: game.ejectedRole || "",
      tieOptions: game.tieOptions || [],
      tieBreakerId: game.tieBreakerId || null,
      selfSabotageTargetId: self && self.role === "fraud" ? game.sabotages[self.id] || null : null,
      selfVoteTargetId: self ? game.votes[self.id] || (game.tieBreakVote?.playerId === self.id ? game.tieBreakVote.targetId : null) : null,
      voteTally: voteTally(room),
      narrationKey: game.narrationKey,
      narrationText: self && self.role === "fraud" ? narrateFraudQuestionForFraud(game.question) : game.narrationText,
      selfAnswer: self && game.answers[self.id] ? publicAnswerMap({ [self.id]: game.answers[self.id] })[self.id] : null,
      answers: {},
      votes: room.phase === "fraud_vote" || room.phase === "fraud_tiebreak" || room.phase === "fraud_ejection" || room.phase === "fraud_scoreboard" || isHost || room.status === "ended" ? game.votes : {},
      reveal: game.reveal || null,
...
function endCup(room) {
  ...
  room.game.result = {
    ...
    title: winners.length === 1
      ? `Congratulations, ${winners[0].name}! You have the highest score in Code & Render Cup!`
      : `Congratulations, ${formatNames(winners.map((player) => player.name))}! You share the highest score in Code & Render Cup!`,
  };
...
function beginFraudQuestion(room, preparedQuestion = null) {
  const game = room.game;
  const question = preparedQuestion || pickUnused(questionBank.debugged, game.used);
  game.questionInCycle = (game.questionInCycle || 0) + 1;
  game.question = { ...question };
  game.pendingQuestion = null;
  game.nextTopic = "";
  game.transitionEnd = 0;
  game.answers = {};
  game.sabotages = {};
  game.votes = {};
  game.tieOptions = [];
  game.tieBreakerId = null;
  game.tieBreakVote = null;
  game.reveal = null;
  room.players.forEach((player) => {
    player.choiceIndex = null;
    player.answered = false;
  ...
  game.narrationText = narrateQuestion(question);
  game.narrationTimeout = Date.now() + 26000;
  if ((game.questionInCycle || 1) === 1) addEvent(room, `Round ${game.round} begins.`, "system");
}
...
function finishFraudAnswer(room) {
  ...
  beginFraudTransition(room);
}

function beginFraudTransition(room) {
  const game = room.game;
  if (room.phase !== "fraud_answer") return;
  room.phase = "fraud_transition";
  game.transitionEnd = Date.now() + 3000;
  if ((game.questionInCycle || 0) >= (game.questionsPerVote || 3)) {
    game.pendingQuestion = null;
    game.nextTopic = "Voting";
  } else {
    game.pendingQuestion = pickUnused(questionBank.debugged, game.used);
    game.nextTopic = game.pendingQuestion.topic;
  }
}

function beginFraudScoreboard(room) {
  const game = room.game;
  if (room.phase !== "fraud_ejection") return;
...
function beginFraudVote(room) {
  const game = room.game;
  if (room.phase !== "fraud_transition") return;
...
function chooseFraudEjection(room) {
  const game = room.game;
  const active = room.players.filter((player) => !player.votedOut);
  if (!active.length) return { ejected: null, tied: [] };
  const tally = voteTally(room);
  const entries = Object.entries(tally).sort((a, b) => b[1] - a[1]);
  if (!entries.length) return { ejected: pick(active), tied: [] };
  const high = entries[0][1];
  const tied = entries.filter((entry) => entry[1] === high).map(([id]) => id);
  if (tied.length > 1) return { ejected: null, tied };
  return { ejected: room.players.find((player) => player.id === entries[0][0]) || null, tied: [] };
}

function beginFraudTieBreak(room, tiedIds) {
  const game = room.game;
  const active = room.players.filter((player) => !player.votedOut);
  const preferredBreakers = active.filter((player) => !tiedIds.includes(player.id));
  const tieBreaker = pick(preferredBreakers.length ? preferredBreakers : active);
  room.phase = "fraud_tiebreak";
  game.tieOptions = tiedIds;
  game.tieBreakerId = tieBreaker ? tieBreaker.id : null;
  game.tieBreakVote = null;
  game.tieBreakEnd = Date.now() + 20000;
}

function finishFraudVote(room) {
  const game = room.game;
  if (room.phase !== "fraud_vote") return;
  const { ejected, tied } = chooseFraudEjection(room);
  if (tied.length) {
    beginFraudTieBreak(room, tied);
    return;
  }
  beginFraudEjection(room, ejected);
}

function finishFraudTieBreak(room) {
  const game = room.game;
  if (room.phase !== "fraud_tiebreak") return;
  const targetId = game.tieBreakVote?.targetId || pick(game.tieOptions || []);
  const ejected = room.players.find((player) => player.id === targetId && !player.votedOut) || null;
  beginFraudEjection(room, ejected);
}

function beginFraudEjection(room, ejected) {
  const game = room.game;
  if (ejected) {
    ejected.votedOut = true;
    game.ejectedId = ejected.id;
    game.ejectedRole = ejected.role;
  ...
  room.phase = "fraud_ejection";
  game.ejectionEnd = Date.now() + 5500;
}

function finishFraudScoreboard(room) {
  const game = room.game;
  if (room.phase !== "fraud_scoreboard") return;
  if (game.ejectedRole === "fraud") {
    const ejected = room.players.find((player) => player.id === game.ejectedId);
    endFraud(room, "students", `Congratulations, Innocent Students! ${ejected ? `${ejected.name} was the Fraud.` : "The Fraud was caught."}`);
    return;
  }
  const fraudAlive = room.players.some((player) => player.role === "fraud" && !player.votedOut);
  if (!fraudAlive) {
    endFraud(room, "students", "Congratulations, Innocent Students! The Fraud has been removed.");
    return;
  }
  if (game.round >= game.maxRounds) {
    endFraud(room, "fraud", "Congratulations, The Fraud! The impostor survived the school day.");
    return;
  }
...
function tickFraud(room) {
  ...
  if (room.phase === "fraud_transition" && now >= game.transitionEnd) {
    if (game.pendingQuestion) beginFraudQuestion(room, game.pendingQuestion);
    else beginFraudVote(room);
    return;
  }
  if (room.phase === "fraud_tiebreak" && now >= game.tieBreakEnd) {
    finishFraudTieBreak(room);
    return;
  }
  if (room.phase === "fraud_ejection" && now >= game.ejectionEnd) {
    beginFraudScoreboard(room);
    return;
  }
  if (room.phase === "fraud_scoreboard" && now >= game.scoreboardEnd) {
    finishFraudScoreboard(room);
    return;
  }
...
function submitMilesAnswer(room, player, choiceIndex) {
  ...
  const now = Date.now();
  const safe = !player.alive || isMilesLookingAway(game, now);
...
function narrateFraudQuestionForFraud(question) {
  if (!question) return "";
  return `${question.topic}. ${question.question}. Answers redacted.`;
}
...
function submitVote(room, player, targetId) {
  tickRoom(room);
  if (room.mode !== "fraud" || (room.phase !== "fraud_vote" && room.phase !== "fraud_tiebreak")) throw new Error("Voting is not open.");
  if (player.votedOut) throw new Error("You were voted out.");
  if (room.phase === "fraud_tiebreak") {
    if (room.game.tieBreakerId !== player.id) throw new Error("Only the tie breaker can vote right now.");
    const tieOptions = (room.game.tieOptions || []).filter((id) => id !== player.id);
    const target = room.players.find((item) => item.id === targetId && !item.votedOut && tieOptions.includes(item.id));
    if (!target) throw new Error("Choose one of the tied players.");
    room.game.tieBreakVote = { playerId: player.id, targetId: target.id };
    finishFraudTieBreak(room);
    return;
  }
