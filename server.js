...
const TUTORIALS = {
  ...
  fraud: {
    ...
    bullets: [
      ...
      "During the answer timer, The Fraud can sabotage another player's answer.",
      "After answers are revealed, everyone votes for the player they suspect.",
...
function sanitizeGame(room, self, isHost) {
  ...
  if (room.mode === "fraud") {
    ...
    return {
      ...
      narrationText: game.narrationText,
      selfAnswer: self ? game.answers[self.id] || null : null,
      answers: room.phase === "fraud_reveal" || isHost || room.status === "ended" ? game.answers : {},
...
function finishFraudAnswer(room) {
  ...
  for (const [fraudId, targetId] of Object.entries(game.sabotages)) {
    const target = room.players.find((player) => player.id === targetId && !player.votedOut);
    if (!target) continue;
    ...
    const fraud = room.players.find((player) => player.id === fraudId);
    addEvent(room, `${target.name}'s answer was manipulated.`, "fraud", target.id);
    if (fraud) addEvent(room, `${fraud.name} sabotaged ${target.name}.`, "fraud");
  }
  ...
  for (const player of active) {
    ...
    results[player.id] = {
      choiceIndex: answer ? answer.choiceIndex : null,
      correct: Boolean(answer && answer.choiceIndex === correctIndex),
      sabotagedBy: answer ? answer.sabotagedBy || null : null,
      role: player.role,
    };
  }
...
function beginFraudVote(room) {
  ...
  addEvent(room, "Voting phase opened.", "vote");
}
...
function submitMilesAnswer(room, player, choiceIndex) {
  ...
  if (!safe) {
    ...
    addEvent(room, `${player.name} got caught by Maam Miles.`, "wrong", player.id);
  ...
  const allAliveSafe = alivePlayers.every((item) => game.answers[item.id] && game.answers[item.id].correct && game.answers[item.id].safe);
  if (allAliveSafe) finishMilesRound(room, "Everyone survived.", false);
}
...
function submitSabotage(room, player, targetId) {
  ...
  if (!target) throw new Error("Choose another active player.");
  room.game.sabotages[player.id] = target.id;
  addEvent(room, `${player.name} prepared a sabotage.`, "fraud", player.id);
}
