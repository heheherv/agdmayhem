      ...
      <main class="lobby-panel host-lobby">
        <section>
          ...
          <div class="join-links">
            ...
            <div class="join-link">${escapeHtml(room.links.local)}</div>
            ${room.links.lan.map((url) => `<div class="join-link">${escapeHtml(url)}</div>`).join("")}
          </div>
...
function renderWheel(game) {
  ...
  const finalRotation = -(chosenIndex * segment + segment / 2);
...
function bindActions() {
  ...
  document.querySelectorAll("[data-field]").forEach((input) => {
    input.addEventListener("input", () => {
      ...
      if (input.dataset.field === "name") state.playerName = input.value;
      input.value = input.dataset.field === "code" ? state.code : state.playerName;
    });
  });
}
