  ...
  if (game.phase === "fraud_scoreboard") {
    return html`
      <main class="game-layout">
        <section class="stage">
          <section class="question-panel">
            <div class="question-topic">Score Reveal</div>
            <h2 class="question-text">Class scores before the vote</h2>
            ${renderFraudScoreboard(room)}
            <p class="muted">Voting opens in ${secondsLeft(game.scoreboardEnd)} seconds.</p>
          </section>
        </section>
        ${renderSidePanel(room)}
      </main>
  ...
  if (game.phase === "fraud_answer" && self?.role === "fraud") {
    return html`
      ...
      <div class="target-grid">
        ${active.filter((player) => player.canSabotageTarget).map((player) => `<button data-action="sabotage" data-target="${player.id}">${escapeHtml(player.name)}</button>`).join("") || `<div class="muted">No innocent targets available.</div>`}
      </div>
    `;
  }
