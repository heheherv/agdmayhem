...
function avatarMotionStyle(seed) {
  const hash = hashSeed(seed);
  const duration = (1.15 + (hash % 8) * 0.12).toFixed(2);
  const delay = ((hash % 11) * 0.13).toFixed(2);
  const x = 1 + (hash % 4);
  const y = 2 + ((hash >> 3) % 5);
  const rot = 1 + ((hash >> 5) % 4);
  return `style="--wiggle-duration:${duration}s; --wiggle-delay:-${delay}s; --wiggle-x:${x}px; --wiggle-y:${y}px; --wiggle-rot:${rot}deg;"`;
}
...
function hostBackControls(label = "Back to Main Menu") {
  if (state.role !== "host") return "";
  return `<div class="host-controls"><button class="secondary-button" data-action="menu">${label}</button></div>`;
}
    ...
    <div class="screen">
      ...
      <main class="join-panel">
        ...
        <div class="form-grid">
          ...
          <div class="wide">
            ...
            <div class="avatar-grid">
              ${AVATARS.map((avatar) => {
                ...
                return `<button class="avatar-choice ${selected ? "selected" : ""} ${isTaken ? "taken" : ""}" data-action="avatar" data-avatar="${avatar}" ${avatarMotionStyle(avatar)} ${isTaken ? "disabled" : ""}>
          ...
          <button class="wide secondary-button" data-action="menu">Back</button>
        </div>
        ${state.error ? `<div class="error">${escapeHtml(state.error)}</div>` : ""}
      </main>
    </div>
    ...
    <div class="screen">
      ...
      <main class="lobby-panel host-lobby">
        <section>
          ...
          <button class="secondary-button" data-action="menu">Back</button>
...
function renderRoster(players) {
  ...
  return players.map((player) => html`
    <div class="player-card ${player.alive === false || player.votedOut ? "out" : ""}" ${avatarMotionStyle(`${player.avatar}-${player.name}`)}>
      <img class="avatar-img" src="${asset(player.avatar)}" alt="${escapeHtml(player.name)}" />
      <div>
        <div class="player-name">${escapeHtml(player.name)}</div>
        <div class="player-meta">${player.playAgain ? "Wants replay" : player.score ? `${player.score} pts` : player.role ? escapeHtml(player.role) : "Ready"}</div>
      </div>
    </div>
  `).join("");
}
    ...
    <main class="game-layout">
      <section class="stage">
        <section class="question-panel tutorial-panel">
          ...
          <p class="muted">${state.role === "host" ? "Host skip starts the game for everyone." : `${tutorial.skippedCount || 0}/${tutorial.totalSkips || room.players.length + 1} ready. The game starts when everyone is ready or the host skips.`}</p>
        </section>
      </section>
...
function renderFraud(room) {
  ...
  if (game.phase === "fraud_transition") return renderFraudTransition(room);
  if (game.phase === "fraud_vote" || game.phase === "fraud_tiebreak") return renderFraudVoting(room, active);
  if (game.phase === "fraud_ejection") return renderFraudEjection(room);
  if (game.phase === "fraud_scoreboard") {
    return html`
      <main class="game-layout">
        <section class="stage">
          <section class="question-panel">
            <div class="question-topic">End of Round ${game.round}</div>
            <h2 class="question-text">Class scores</h2>
            ${renderFraudScoreboard(room)}
            <p class="muted">${secondsLeft(game.scoreboardEnd)} seconds until the next screen.</p>
          </section>
        </section>
        ${renderSidePanel(room)}
      </main>
      ...
      <section class="stage">
        <section class="question-panel">
          <div style="display:flex; gap:.6rem; flex-wrap:wrap; align-items:center; margin-bottom:.65rem;">
            ...
            <span class="question-topic">Evidence ${game.questionInCycle || 1}/${game.questionsPerVote || 3}</span>
...
function renderFraudTransition(room) {
  const game = room.game;
  return html`
    <main class="game-layout">
      <section class="stage">
        <section class="question-panel">
          <div class="question-topic">Next Up</div>
          <h2 class="question-text">${escapeHtml(game.nextTopic || "Voting")}</h2>
          <div class="big-count">${secondsLeft(game.transitionEnd)}s</div>
          <p class="muted">No answer reveal. Keep your evidence in mind.</p>
        </section>
      </section>
      ${renderSidePanel(room)}
    </main>
...
function renderFraudVoting(room, active) {
  const game = room.game;
  const self = room.self;
  const tiedPlayers = (game.tieOptions || []).map((id) => room.players.find((player) => player.id === id)).filter(Boolean);
  const tieBreaker = room.players.find((player) => player.id === game.tieBreakerId);
  const canVote = state.role === "player" && !self?.votedOut;
  const tiebreakerTurn = game.phase === "fraud_tiebreak" && self?.id === game.tieBreakerId;
  return html`
    <main class="game-layout">
      <section class="stage">
        <section class="question-panel">
          <div class="question-topic">${game.phase === "fraud_tiebreak" ? "Tie Breaker" : "Class Vote"}</div>
          <h2 class="question-text">${game.phase === "fraud_tiebreak" ? `${escapeHtml(tieBreaker?.name || "A student")} decides the tie` : "Vote one student out"}</h2>
          ${game.phase === "fraud_tiebreak" ? `<p class="muted">Tied players: ${tiedPlayers.map((player) => escapeHtml(player.name)).join(", ")}. ${secondsLeft(game.tieBreakEnd)} seconds left.</p>` : `<p class="muted">Voting ends in ${secondsLeft(game.voteEnd)} seconds.</p>`}
          ${renderVoteBoard(room)}
          ${game.phase === "fraud_tiebreak" ? html`
            ${tiebreakerTurn ? html`
              <div class="vote-grid">
                ${tiedPlayers.filter((player) => player.id !== self.id).map((player) => `<button class="${game.selfVoteTargetId === player.id ? "selected-target" : ""}" data-action="vote" data-target="${player.id}">${escapeHtml(player.name)}</button>`).join("")}
              </div>
            ` : `<p class="muted">${escapeHtml(tieBreaker?.name || "The tie breaker")} is choosing who leaves.</p>`}
          ` : html`
            ${canVote ? html`
              <div class="vote-grid">
                ${active.filter((player) => player.id !== self.id).map((player) => `<button class="${game.selfVoteTargetId === player.id ? "selected-target" : ""}" data-action="vote" data-target="${player.id}">${escapeHtml(player.name)}</button>`).join("")}
              </div>
            ` : ""}
          `}
        </section>
...
function renderFraudEjection(room) {
  const game = room.game;
  const ejected = room.players.find((player) => player.id === game.ejectedId);
  const roleLine = ejected
    ? game.ejectedRole === "fraud"
      ? `${ejected.name} was the Fraud.`
      : `${ejected.name} was an Innocent Student.`
    : "No one left the classroom.";
  return html`
    <main class="game-layout">
      <section class="stage">
        <section class="question-panel cutscene-panel">
          <div class="question-topic">Kicked Out</div>
          <h2 class="question-text">${escapeHtml(ejected ? `${ejected.name} leaves the classroom` : "No ejection")}</h2>
          ${ejected ? `<img class="cutscene-avatar" src="${asset(ejected.avatar)}" alt="${escapeHtml(ejected.name)}" ${avatarMotionStyle(`${ejected.avatar}-${ejected.name}-ejected`)} />` : ""}
          <p class="cutscene-line">${escapeHtml(roleLine)}</p>
          <p class="muted">${secondsLeft(game.ejectionEnd)} seconds until scores.</p>
        </section>
      </section>
      ${renderSidePanel(room)}
    </main>
...
function renderFraudScoreboard(room) {
  return html`
    <div class="fraud-scoreboard">
      ${[...room.players].sort((a, b) => (b.score || 0) - (a.score || 0)).map((player) => html`
        <div class="fraud-score-row ${player.votedOut ? "out" : ""}" ${avatarMotionStyle(`${player.avatar}-${player.name}-score`)}>
...
function renderVoteBoard(room) {
  const game = room.game;
  const active = room.players.filter((player) => !player.votedOut);
  return html`
    <div class="vote-board">
      ${active.map((voter) => {
        const target = room.players.find((player) => player.id === game.votes?.[voter.id]);
        return `<div class="vote-row"><span>${escapeHtml(voter.name)}</span><strong>${target ? escapeHtml(target.name) : "..."}</strong></div>`;
      }).join("")}
    </div>
  `;
}
  ...
  if (game.phase === "fraud_answer" && self?.role === "fraud") {
    return html`
      ...
      <div class="target-grid">
        ${active.filter((player) => player.canSabotageTarget).map((player) => `<button class="${game.selfSabotageTargetId === player.id ? "selected-target" : ""}" data-action="sabotage" data-target="${player.id}">${escapeHtml(player.name)}</button>`).join("") || `<div class="muted">No innocent targets available.</div>`}
      </div>
    `;
  }
          ...
          <div class="desks">
            ${room.players.map((player) => html`
              <div class="desk ${player.alive ? "" : "out"}">
                <img src="${asset(player.avatar)}" alt="${escapeHtml(player.name)}" ${avatarMotionStyle(`${player.avatar}-${player.name}-desk`)} />
                <div>
                  <div class="player-name">${escapeHtml(player.name)}</div>
                  <span class="status-pill">${player.alive ? (player.seenAttempts ? `Seen ${player.seenAttempts}/3` : "In class") : `Out ${player.comeback}/2`}</span>
                </div>
              </div>
            `).join("")}
          </div>
...
function renderChoiceAvatars(room, answerMap, choiceIndex) {
  ...
  return `<div class="choice-avatars">${players.map((player) => `<img class="mini-face" title="${escapeHtml(player.name)}" src="${asset(player.avatar)}" alt="${escapeHtml(player.name)}" ${avatarMotionStyle(`${player.avatar}-${player.name}-mini`)} />`).join("")}</div>`;
}
      ...
      <div class="leaderboard">
        ${[...room.players].sort((a, b) => (b.score || 0) - (a.score || 0)).map((player) => html`
          <div class="score-row ${player.alive === false || player.votedOut ? "out" : ""}">
            <img src="${asset(player.avatar)}" alt="${escapeHtml(player.name)}" ${avatarMotionStyle(`${player.avatar}-${player.name}-side`)} />
            <span class="name">${escapeHtml(player.name)}</span>
            <span>${room.mode === "cup" || room.mode === "fraud" ? `${player.score || 0}` : player.votedOut ? "Voted" : player.alive === false ? "Out" : player.seenAttempts ? `Seen ${player.seenAttempts}/3` : "In"}</span>
          </div>
        `).join("")}
      </div>
...
function renderConfetti() {
  const pieces = Array.from({ length: 34 }, (_, index) => {
    ...
    const delay = ((index * 0.09) % 1.4).toFixed(2);
    const duration = (1.6 + (index % 7) * 0.18).toFixed(2);
    const drift = ((index % 9) - 4) * 12;
    return `<span style="--left:${left}%; --delay:${delay}s; --duration:${duration}s; --drift:${drift}px;"></span>`;
  }).join("");
...
function renderWinner(room) {
  ...
  const replayRequests = room.players.filter((player) => player.playAgain);
  let winners = [];
    ...
    <div class="screen">
      ...
      <main class="winner-screen">
        <section class="question-panel">
          ...
          <div class="big-avatars">${winners.map((player) => `<img src="${asset(player.avatar)}" alt="${escapeHtml(player.name)}" ${avatarMotionStyle(`${player.avatar}-${player.name}-winner`)} />`).join("")}</div>
          ${state.role === "host" ? html`
            <p class="muted">${replayRequests.length}/${room.players.length} player${replayRequests.length === 1 ? "" : "s"} want to play again.</p>
            <div class="end-actions">
              <button data-action="replay">Replay Same Mode</button>
              <button class="secondary-button" data-action="menu">Back to Main Menu</button>
            </div>
          ` : html`
            <div class="end-actions">
              <button data-action="play-again" ${room.self?.playAgain ? "disabled" : ""}>${room.self?.playAgain ? "Play Again Sent" : "Play Again"}</button>
              <button class="secondary-button" data-action="new-code">Enter New Room Code</button>
            </div>
  ...
  document.querySelectorAll("[data-action]").forEach((element) => {
    element.addEventListener("click", async () => {
      ...
      try {
        ...
        if (action === "replay") {
          const data = await api.request(`/api/rooms/${state.room.code}/replay`, { method: "POST", body: {} });
          receiveRoom(data);
          audio.startLoop(state.room.mode);
          return;
        }
        if (action === "play-again") {
          const data = await api.request(`/api/rooms/${state.room.code}/play-again`, {
            method: "POST",
